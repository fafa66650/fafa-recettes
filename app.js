...
const equipmentOptions=['Cuisine classique','Four','Poêle','Blender / mixeur'];
const equipmentDisplay={'Cuisine classique':'Ustensiles & four'};
const deviceLabels={blender:'Blender / mixeur'};
const categoryIcon={
...
function applyQuickFilter(type,btn){
  ...
  if(type==='mode'){let eq='';if(state.prefs.mode==='classic')eq='Cuisine classique';else{const devices=['Blender / mixeur'];eq=devices.find(d=>(state.prefs.equipment||[]).includes(d))||'';}state.filters.equipment=state.filters.equipment===eq?'':eq;$('#equipmentFilter').value=state.filters.equipment;}
...
function renderRecipeLibrary(){
  ...
  const cbox=$('#recipeCuisineHighlights');if(cbox){
    const groups=CUISINE_GROUPS.map(g=>[g,state.recipes.filter(r=>(r.regionCuisine||r.cuisineRegion)===g).length]).filter(x=>x[1]);
...
function generateSurprise(){
  ...
  const eq=state.prefs.mode==='classic'?'Cuisine classique':'';
...
function recipeCard(r,extra=''){
  ...
  const visual=image?`<img class="${fit}" src="${escapeHtml(image)}" alt="${escapeHtml(r.name)}" loading="lazy">`:`<div class="recipe-placeholder"><strong>${escapeHtml(r.category)}</strong><small>${escapeHtml(label)}</small><em>Visuel à venir</em></div>`;
  return `<article class="recipe-card recipe-card-clickable" ${extra}><button class="recipe-card-main" data-open-card="${r.id}" aria-label="Ouvrir ${escapeHtml(r.name)}"><div class="recipe-image ${image?'has-image':'no-image'}">${visual}</div><div class="recipe-body"><span class="recipe-category-line">${escapeHtml(label)}</span><h3 class="recipe-title">${escapeHtml(r.name)}</h3><div class="recipe-meta recipe-meta-simple"><span>⏱ ${times||'—'} min</span>${r.difficulty?`<span>${escapeHtml(r.difficulty)}</span>`:''}<span>${escapeHtml(supportBadge(r))}</span></div></div></button><button class="fav-btn ${fav?'active':''}" data-fav="${r.id}" aria-label="${fav?'Retirer des':'Ajouter aux'} favoris">${fav?'♥':'♡'}</button></article>`;
}
...
function openRecipe(id,open=true){
  ...
  if(open){
    ...
    state.currentDrawerMode='classic';
  ...
  const modeText=state.currentDrawerMode==='classic'?'casserole, poêle, four et ustensiles':(deviceLabels[state.currentDeviceMode]||'appareil sélectionné');
  ...
  <section class="drawer-section nutrition-section"><span class="kicker">REPÈRES NUTRITIONNELS</span><h3>Par portion / 1 personne</h3>${nutritionHtml}${allergens.length?`<div class="allergen-box"><strong>Contient / peut contenir :</strong> ${escapeHtml(allergens.join(', '))}</div>`:''}</section>
  ...
  ${((r.tips||[]).length||(r.substitutions||[]).length)?`<section class="drawer-section full-width-section"><span class="kicker">ASTUCES</span><h3>Conseils & adaptations</h3><div class="steps-list">${(r.tips||[]).map(t=>`<div class="step-box"><p>${escapeHtml(t)}</p></div>`).join('')}${(r.substitutions||[]).slice(0,6).map(t=>`<div class="step-box"><p>↔ ${escapeHtml(t)}</p></div>`).join('')}</div></section>`:''}
