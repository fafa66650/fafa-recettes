...
function recipeCard(r,extra=''){
  ...
  return `<article class="recipe-card recipe-card-clickable" data-open-card="${r.id}" tabindex="0" role="button" aria-label="Ouvrir ${escapeHtml(r.name)}" ${extra}><div class="recipe-image ${image?'':'no-image'}">${image?`<img src="${escapeHtml(image)}" alt="${escapeHtml(r.name)}" loading="lazy">`:`<div class="recipe-placeholder"><span>${r.emoji||categoryIcon[r.category]||'🍽️'}</span><strong>${escapeHtml(r.subtype||r.category)}</strong><small>${escapeHtml(mains)}</small></div>`}<button class="fav-btn ${fav?'active':''}" data-fav="${r.id}" aria-label="${fav?'Retirer des':'Ajouter aux'} favoris">${fav?'♥':'♡'}</button></div><div class="recipe-body"><span class="recipe-category-line">${escapeHtml(r.category)}</span><h3 class="recipe-title">${escapeHtml(r.name)}</h3><div class="recipe-meta recipe-meta-simple"><span>⏱ ${times||'—'} min</span><span>${escapeHtml(supportBadge(r))}</span></div></div></article>`;
}
