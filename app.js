...
const DEFAULT_PREFS={mode:'tm6',guided:true,household:4,restrictions:[],equipment:['TM6','Cuisine classique'],favorites:[],favoriteMenus:[],menuOverrides:{},pantry:[],pantryFavorites:[],pantryRecent:[],shopping:[],manualShopping:[],shoppingDone:[],planning:{},recipeNotes:{},leftovers:{},customRecipes:[],collections:{'À tester':[]},lastPage:'home',recentCategories:[],recentRecipes:[]};
const state={
  recipes:[],menus:[],
  prefs:{...DEFAULT_PREFS},
...
function loadPrefs(){
  try{for(let i=localStorage.length-1;i>=0;i--){const k=localStorage.key(i)||'';if(/^fafatraining-recettes-v\d+/i.test(k))localStorage.removeItem(k);}}catch{}
  ...
  state.prefs.planning=state.prefs.planning||{};state.prefs.household=Math.max(1,Math.min(12,Number(state.prefs.household)||4));state.prefs.recipeNotes=state.prefs.recipeNotes||{};state.prefs.leftovers=state.prefs.leftovers||{};state.prefs.customRecipes=state.prefs.customRecipes||[];state.prefs.collections=state.prefs.collections||{'À tester':[]};
}
...
function openCatalogDB(){return new Promise((resolve,reject)=>{if(!('indexedDB' in window))return resolve(null);const req=indexedDB.open(DB_NAME,1);req.onupgradeneeded=()=>{const db=req.result;if(!db.objectStoreNames.contains('catalog'))db.createObjectStore('catalog');};req.onsuccess=()=>resolve(req.result);req.onerror=()=>resolve(null);});}
async function cacheCatalog(key,value){try{const db=await openCatalogDB();if(!db)return;await new Promise((res,rej)=>{const tx=db.transaction('catalog','readwrite');tx.objectStore('catalog').put(value,key);tx.oncomplete=res;tx.onerror=rej;});db.close();}catch{}}
async function readCachedCatalog(key){try{const db=await openCatalogDB();if(!db)return null;const v=await new Promise(res=>{const tx=db.transaction('catalog','readonly');const rq=tx.objectStore('catalog').get(key);rq.onsuccess=()=>res(rq.result||null);rq.onerror=()=>res(null);});db.close();return v;}catch{return null;}}
async function loadCatalogFile(url,key){try{const resp=await fetch(url,{cache:'no-store'});if(!resp.ok)throw new Error('network');const data=await resp.json();cacheCatalog(key,data);return data;}catch{const cached=await readCachedCatalog(key);if(cached)return cached;throw new Error('catalog unavailable');}}

async function init(){
  loadPrefs();
  try{
    [state.recipes,state.menus]=await Promise.all([loadCatalogFile('recipes.json?build=final','recipes'),loadCatalogFile('menus.json?build=final','menus')]);
  ...
  navigate(allowed.includes(requested)?requested:(allowed.includes(state.prefs.lastPage)?state.prefs.lastPage:'home'),false,false);
}

function buildStaticUI(){
  if($('#homeModeSelect')){$('#homeModeSelect').value=state.prefs.mode;$('#homeModeHint').textContent=MODE_META[state.prefs.mode]?.desc||'Ton choix est mémorisé.';}
...
function bindEvents(){
  ...
  if($('#homeModeSelect'))$('#homeModeSelect').onchange=e=>setPreferredMode(e.target.value);
  ...
  if($('#surpriseTopBtn'))$('#surpriseTopBtn').onclick=()=>openSurprise(true);$('#homeSurpriseBtn').onclick=()=>openSurprise(true);
  ...
  document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeRecipe();closeMenuDrawer();openFilterDrawer(false);openSettings(false);openSurprise(false);openPlanAssign(false);openCustomRecipe(false);openKitchenGuide(false);openCollections(false);closeExport();closeCook();}});
  ...
  if($('#openCollectionsBtn'))$('#openCollectionsBtn').onclick=()=>openCollections(true);$$('[data-close-collections]').forEach(x=>x.onclick=()=>openCollections(false));if($('#addCollectionBtn'))$('#addCollectionBtn').onclick=addCollection;if($('#newCollectionName'))$('#newCollectionName').addEventListener('keydown',e=>{if(e.key==='Enter')addCollection();});
  if($('#customManualTab'))$('#customManualTab').onclick=()=>setCustomImportMode('manual');if($('#customPasteTab'))$('#customPasteTab').onclick=()=>setCustomImportMode('paste');if($('#parseImportedRecipeBtn'))$('#parseImportedRecipeBtn').onclick=parseImportedRecipeText;
  if($('#customImageInput'))$('#customImageInput').onchange=readCustomImage;if($('#cookSpeakBtn'))$('#cookSpeakBtn').onclick=speakCookStep;
  bindDynamicUI();
}
...
let customImageData='';
function setCustomImportMode(mode){const paste=mode==='paste';$('#customPastePanel')?.classList.toggle('hidden',!paste);$('#customManualPanel')?.classList.toggle('hidden',paste);$('#customPasteTab')?.classList.toggle('active',paste);$('#customManualTab')?.classList.toggle('active',!paste);}
function parseImportedRecipeText(){const raw=$('#customImportText')?.value.trim();if(!raw)return toast('Colle d’abord le texte d’une recette');const lines=raw.split(/\n+/).map(x=>x.trim()).filter(Boolean);if(!lines.length)return;$('#customName').value=lines[0].replace(/^#+\s*/,'').slice(0,120);const servLine=lines.find(x=>/personne|portion/i.test(x));const sm=servLine?.match(/(\d+)\s*(?:personne|portion)/i);if(sm)$('#customServings').value=Math.min(12,Math.max(1,Number(sm[1])));let prepIndex=lines.findIndex(x=>/^(préparation|preparation|étapes|etapes|instructions|méthode|methode)/i.test(x));const candidates=prepIndex>=0?lines.slice(1,prepIndex):lines.slice(1);const ing=candidates.filter(x=>/^[-•]?\s*\d+(?:[.,]\d+)?\s*(?:g|kg|ml|cl|l|c\.|cuill|pi[eè]ce|gousse|tranche|pinc)/i.test(x));if(ing.length)$('#customIngredients').value=ing.map(x=>x.replace(/^[-•]\s*/,'' )).join('\n');const steps=prepIndex>=0?lines.slice(prepIndex+1).filter(x=>!/^ingr[eé]dients/i.test(x)):lines.filter(x=>/^\d+[.)-]/.test(x));if(steps.length)$('#customSteps').value=steps.map(x=>x.replace(/^\d+[.)-]\s*/,'' )).join('\n');setCustomImportMode('manual');toast('Texte analysé : vérifie puis complète la fiche');}
function readCustomImage(e){const f=e.target.files?.[0];customImageData='';if(!f)return;if(f.size>900000){toast('Choisis une image de moins de 900 Ko pour une recette personnelle');e.target.value='';return;}const rd=new FileReader();rd.onload=()=>{customImageData=String(rd.result||'');toast('Photo prête à être enregistrée');};rd.readAsDataURL(f);}
function openCollections(open){const d=$('#collectionsDrawer');if(!d)return;d.classList.toggle('open',open);d.setAttribute('aria-hidden',String(!open));if(open){renderCollections();document.body.style.overflow='hidden';}else if(!$('#recipeDrawer').classList.contains('open'))document.body.style.overflow='';}
function addCollection(){const input=$('#newCollectionName'),name=input?.value.trim();if(!name)return;if(!state.prefs.collections[name])state.prefs.collections[name]=[];input.value='';savePrefs();renderCollections();toast('Collection créée');}
function toggleRecipeCollection(recipeId,name){const arr=state.prefs.collections[name]||[];state.prefs.collections[name]=arr.includes(recipeId)?arr.filter(x=>x!==recipeId):[...arr,recipeId];savePrefs();renderCollections();if(state.currentRecipe?.id===recipeId)openRecipe(recipeId,false);}
function renderCollections(){const box=$('#collectionsList');if(!box)return;const entries=Object.entries(state.prefs.collections||{});box.innerHTML=entries.length?entries.map(([name,ids])=>`<section class="collection-card"><div><strong>${escapeHtml(name)}</strong><small>${ids.length} recette${ids.length>1?'s':''}</small></div><div class="collection-recipes">${ids.slice(0,6).map(id=>{const r=state.recipes.find(x=>x.id===id);return r?`<button data-open-collection-recipe="${id}">${escapeHtml(r.name)}</button>`:'';}).join('')||'<span class="small-note">Collection vide</span>'}</div><button class="collection-delete" data-delete-collection="${escapeHtml(name)}" aria-label="Supprimer la collection">Supprimer</button></section>`).join(''):'<div class="shopping-empty">Crée ta première collection pour ranger tes recettes autrement que dans les favoris.</div>';box.querySelectorAll('[data-open-collection-recipe]').forEach(b=>b.onclick=()=>{openCollections(false);openRecipe(b.dataset.openCollectionRecipe);});box.querySelectorAll('[data-delete-collection]').forEach(b=>b.onclick=()=>{delete state.prefs.collections[b.dataset.deleteCollection];savePrefs();renderCollections();});}
function openCustomRecipe(open){
  ...
  if(open){$('#customServings').value=state.prefs.household||4;setCustomImportMode('manual');document.body.style.overflow='hidden';}
...
function saveCustomRecipe(){
  ...
  const id='perso-'+slug(name)+'-'+Date.now();const rec={id,name,category:cat,subtype:'Mes recettes',color:'#75d429',emoji:'🍽️',servings:serv,difficulty:'',tags:['Recette personnelle'],ingredients,modes:{classic:stepLines.map((text,i)=>({title:`Étape ${i+1}`,text,durationSec:0}))},tips:[],goalTags:[],dietTags:[],allergens:[],substitutions:[],macros:{},styles:['Cuisine maison'],equipment:['Cuisine classique'],prepMin:10,cookMin:0,description:'Recette personnelle enregistrée sur cet appareil.',featured:false,sourceType:'personnelle',nutritionBasis:'Valeurs nutritionnelles non calculées pour cette recette.',nutritionStatus:'missing',tm6Status:'none',image:customImageData||'',cuisine:'Maison',techniques:['mise en place'],storage:'Conserve les restes rapidement au réfrigérateur dans une boîte fermée.',safetyTip:'',deviceModes:{},learningMode:true};
  state.prefs.customRecipes.push(rec);state.recipes.push(rec);customImageData='';savePrefs();buildStaticUI();bindDynamicUI();renderRecipes(true);openCustomRecipe(false);$('#customName').value='';$('#customIngredients').value='';$('#customSteps').value='';toast('Recette personnelle enregistrée');
}
...
function goBack(){
  ...
  if($('#collectionsDrawer')?.classList.contains('open'))return openCollections(false);
...
function applyModeButtons(){$$('[data-mode]').forEach(b=>b.classList.toggle('active',b.dataset.mode===state.prefs.mode));if($('#homeModeSelect')){$('#homeModeSelect').value=state.prefs.mode;if($('#homeModeHint'))$('#homeModeHint').textContent=MODE_META[state.prefs.mode]?.desc||'Ton choix est mémorisé.';}}
...
function filteredRecipes(){
  ...
  if(f.fridgeOnly&&state.prefs.pantry.length){arr=arr.filter(r=>missingIngredientsFor(r).length===0);}
...
function personalizedRecipes(){
  ...
  const tests=[r=>isMainCourseCategory(r.category),r=>['Légumes & accompagnements','Végétarien'].includes(r.category),r=>r.category==='Desserts'||totalTime(r)<=20||['Petit-déjeuner','Entrées & salades'].includes(r.category)];
  const picked=[];for(const test of tests){const x=scored.find(v=>test(v.r)&&!picked.some(p=>p.id===v.r.id)&&!(v.r.category==='Boissons & smoothies'));if(x)picked.push(x.r);}
  for(const x of scored){if(picked.length>=3)break;if(x.r.category==='Boissons & smoothies')continue;if(!picked.some(p=>p.id===x.r.id))picked.push(x.r);}
...
function renderHome(){const forYou=personalizedRecipes();$('#forYouGrid').innerHTML=forYou.map(recipeCard).join('');bindRecipeCards($('#forYouGrid'));renderTodayPlan();}
function renderTodayPlan(){const sec=$('#todayPlanSection'),box=$('#todayPlanCard');if(!sec||!box)return;const jsDay=new Date().getDay();const keyDay={1:'mon',2:'tue',3:'wed',4:'thu',5:'fri',6:'sat',0:'sun'}[jsDay];const entries=['lunch','dinner'].map(slot=>({slot,e:state.prefs.planning?.[`${keyDay}-${slot}`]})).filter(x=>x.e);if(!entries.length){sec.classList.add('hidden');box.innerHTML='';return;}const now=new Date().getHours();let chosen=entries.find(x=>x.slot===(now<14?'lunch':'dinner'))||entries[0];const info=planningEntryLabel(chosen.e);if(!info){sec.classList.add('hidden');return;}sec.classList.remove('hidden');box.innerHTML=`<div><span class="kicker">${chosen.slot==='lunch'?'MIDI':'SOIR'}</span><strong>${escapeHtml(info.title)}</strong><small>${escapeHtml(info.meta)}</small></div><button id="openTodayPlan" class="primary-btn">Ouvrir</button>`;$('#openTodayPlan').onclick=()=>{if(chosen.e.type==='menu')openMenuDrawer(chosen.e.id);else openRecipe(chosen.e.id);};}
...
const MENU_UNIVERSES=[
  ...
  {id:'antigaspi',title:'Packs anti-gaspi',icon:'♻️',desc:'3 repas différents qui réutilisent plusieurs ingrédients pour simplifier les courses.',tags:['Anti-gaspi']}
];
...
function openRecipe(id,open=true){
  ...
  <div class="drawer-grid"><section class="drawer-section ingredients-main"><div class="section-title-row"><div><span class="kicker">AVANT DE COMMENCER</span><h3>Ingrédients</h3></div></div><div class="ingredient-list checklist">${r.ingredients.map(i=>`<label class="ingredient-check"><input type="checkbox"><span>${escapeHtml(i.name)}</span><strong>${formatScaledQty(i,factor)} ${escapeHtml(i.unit||'')}</strong></label>`).join('')}</div></section>
  ...
  if($('#drawerCollectionBtn'))$('#drawerCollectionBtn').onclick=()=>{const names=Object.keys(state.prefs.collections||{});if(!names.length){openCollections(true);return;}const name=prompt('Dans quelle collection ?\n'+names.map((x,i)=>`${i+1}. ${x}`).join('\n'),names[0]);if(name&&state.prefs.collections[name]){toggleRecipeCollection(r.id,name);toast((state.prefs.collections[name]||[]).includes(r.id)?`Ajouté à ${name}`:`Retiré de ${name}`);}};
...
function formatScaledQty(ing,factor){let n=Number(ing.qty||0);const u=norm(ing.unit||''),name=norm(ing.name||'');if(!Number.isFinite(n))return'';let scaled=n*factor;if(/piece|pièce|gousse|tranche|pave|pavé/.test(u)){if(scaled<1)scaled=Math.max(.5,Math.round(scaled*2)/2);else if(scaled<3)scaled=Math.round(scaled*2)/2;else scaled=Math.round(scaled);}else if(/c. a cafe|c. à café|pincee|pincée/.test(u)&&/sel|poivre|paprika|curry|cumin|cannelle|piment|muscade|curcuma|epice|épice/.test(name)){const gentle=1+Math.log(Math.max(1,factor))*.65;scaled=n*(factor>=1?gentle:factor);}return formatQty(scaled);}
...
function pantryGroupFor(name){
  ...
  if(pantryHasAny(h,['oeuf','lait','yaourt','fromage','creme','beurre','ricotta','feta','mozzarella','parmesan','skyr','mascarpone','fromage blanc','petit-suisse','kefir']))return'Œufs, laitages & alternatives';
  if(pantryHasAny(h,['tofu','tempeh','seitan','proteine vegetale','protéine végétale']))return'Protéines végétales';
...
function pantryGroups(all){const groups={};all.forEach(x=>(groups[pantryGroupFor(x)]??=[]).push(x));const order=['Légumes & aromates','Fruits','Viandes & volaille','Poissons & fruits de mer','Œufs, laitages & alternatives','Protéines végétales','Pâtes, riz & céréales','Légumineuses','Fruits à coque & graines','Boissons, café & infusions','Épicerie, sauces & condiments','Herbes & épices','Autres & placard'];const out={};order.forEach(g=>{if(groups[g]?.length)out[g]=groups[g].sort((a,b)=>a.localeCompare(b,'fr'));});return out;}
function pantryGroupIcon(g){return {'Légumes & aromates':'🥕','Fruits':'🍎','Viandes & volaille':'🥩','Poissons & fruits de mer':'🐟','Œufs, laitages & alternatives':'🥚','Protéines végétales':'🌱','Pâtes, riz & céréales':'🌾','Légumineuses':'🫘','Fruits à coque & graines':'🥜','Boissons, café & infusions':'☕','Épicerie, sauces & condiments':'🧂','Herbes & épices':'🌿','Autres & placard':'🫙'}[g]||'🍽️';}
...
function speakCookStep(){if(!('speechSynthesis' in window))return toast('Lecture vocale indisponible sur cet appareil');const c=state.cook,s=c.steps[c.index];if(!s)return;window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(`${s.title}. ${s.text}`);u.lang='fr-FR';u.rate=.92;window.speechSynthesis.speak(u);}
