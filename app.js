...
const RECIPE_ID_ALIASES={"omelette-epinards-champignons":"omelette-champignons-epinards","flan-vanille-coco":"flan-coco-vanille","crevettes-coco-curry":"crevettes-curry-coco","smoothie-banane-cafe-cacao":"smoothie-cafe-banane-cacao","semoule-poulet-legumes-epices-douces":"semoule-poulet-legumes","pates-aubergine-tomate":"pates-aubergine-tomate-basilic","veloute-celeri-pomme":"veloute-celeri-rave-pomme","boulgour-dinde-tomate-menthe":"boulgour-dinde-tomate","pancakes-yaourt-citron":"pancakes-ricotta-citron","tartines-avocat-tomate-uf":"tartines-avocat-uf","porridge-poire-cacao-noisettes":"porridge-cacao-orange-noisettes","porridge-mangue-coco":"porridge-ananas-coco-gingembre","chia-pudding-mangue-citron-vert":"chia-pudding-framboise-vanille","muesli-chaud-pomme-raisins":"bircher-muesli-pomme-raisins","ufs-brouilles-tomate-feta":"menemen-turc","omelette-courgette-basilic":"frittata-courgette-basilic","pois-chiches-dahl-coco-epinards":"lentilles-corail-dahl-coco-epinards","haricots-rouges-dahl-coco-epinards":"lentilles-corail-dahl-coco-epinards","haricots-blancs-dahl-coco-epinards":"lentilles-corail-dahl-coco-epinards","pois-casses-dahl-coco-epinards":"lentilles-corail-dahl-coco-epinards","lentilles-corail-tomate-cumin-legumes":"pois-chiches-tomate-cumin-legumes","haricots-rouges-tomate-cumin-legumes":"pois-chiches-tomate-cumin-legumes","haricots-blancs-tomate-cumin-legumes":"pois-chiches-tomate-cumin-legumes","pois-casses-tomate-cumin-legumes":"pois-chiches-tomate-cumin-legumes","lentilles-corail-paprika-poivrons":"haricots-rouges-paprika-poivrons","pois-chiches-paprika-poivrons":"haricots-rouges-paprika-poivrons","haricots-blancs-paprika-poivrons":"haricots-rouges-paprika-poivrons","pois-casses-paprika-poivrons":"haricots-rouges-paprika-poivrons","lentilles-corail-citron-herbes-boulgour":"pois-chiches-citron-herbes-boulgour","haricots-rouges-citron-herbes-boulgour":"pois-chiches-citron-herbes-boulgour","haricots-blancs-citron-herbes-boulgour":"pois-chiches-citron-herbes-boulgour","pois-casses-citron-herbes-boulgour":"pois-chiches-citron-herbes-boulgour","puree-pois-chiches-citron":"houmous-classique","smoothie-kiwi-maison":"smoothie-kiwi-pomme","smoothie-pomme-maison":"smoothie-kiwi-pomme","lait-cajou-maison":"lait-de-cajou-vanille","veloute-brocoli-leger":"veloute-brocoli-pomme-de-terre"};
...
const CUISINE_GROUPS=['France & Europe','Méditerranée, Maghreb & Levant','Asie','Afrique','Amériques','Îles & Caraïbes','Maison & contemporain'];
...
function buildStaticUI(){
  $('#heroModeButtons').innerHTML=Object.entries(MODE_META).map(([k,v])=>`<button class="mode-card mode-${k}" data-mode="${k}"><strong>${escapeHtml(v.short)}</strong></button>`).join('');
...
function navigate(page,scroll=true,push=true){
  ...
  $('#topBackBtn').classList.toggle('hidden',page==='home');
...
function renderRecipeLibrary(){
  ...
  const cbox=$('#recipeCuisineHighlights');if(cbox){
    const groups=CUISINE_GROUPS.map(g=>[g,state.recipes.filter(r=>r.cuisineRegion===g).length]).filter(x=>x[1]);
    cbox.innerHTML=groups.map(([g,n])=>`<button class="cuisine-family-card" data-library-cuisine-group="${escapeHtml(g)}"><strong>${escapeHtml(g)}</strong><small>${n} recettes</small><b>→</b></button>`).join('');
...
function filteredRecipes(){
  ...
  if(f.cuisineGroup)arr=arr.filter(r=>r.cuisineRegion===f.cuisineGroup);
...
function renderHome(){renderHomeNextMeal();const forYou=personalizedRecipes();$('#forYouGrid').innerHTML=forYou.map(recipeCard).join('');bindRecipeCards($('#forYouGrid'));}
function renderOrganize(){
  const box=$('#organizeNextMeal');if(!box)return;const entries=Object.entries(state.prefs.planning||{}).filter(([,e])=>e);const count=entries.length;
  if(!count){box.classList.remove('hidden');box.innerHTML=`<div><span class="kicker">MA SEMAINE</span><strong>Aucun repas planifié</strong><small>Commence par choisir un menu ou ajoute directement une recette au planning.</small></div><button class="primary-btn" data-nav-dynamic="planning">Construire ma semaine</button>`;}
  else{box.classList.remove('hidden');box.innerHTML=`<div><span class="kicker">MA SEMAINE</span><strong>${count} repas planifié${count>1?'s':''} sur 14</strong><small>Tu peux continuer à remplir la semaine ou préparer les courses.</small></div><button class="primary-btn" data-nav-dynamic="planning">Voir mon planning</button>`;}
...
function recipeCard(r,extra=''){
  ...
  return `<article class="recipe-card recipe-card-clickable" data-open-card="${r.id}" tabindex="0" role="button" aria-label="Ouvrir ${escapeHtml(r.name)}" ${extra}><div class="recipe-image ${image?'has-image':'no-image'}">${image?`<img class="${fit}" src="${escapeHtml(image)}" alt="${escapeHtml(r.name)}" loading="lazy">`:`<div class="recipe-placeholder"><span>${categoryIcon[r.category]||'🍽️'}</span><strong>${escapeHtml(r.category)}</strong><small>${escapeHtml(label)}</small></div>`}<button class="fav-btn ${fav?'active':''}" data-fav="${r.id}" aria-label="${fav?'Retirer des':'Ajouter aux'} favoris">${fav?'♥':'♡'}</button></div><div class="recipe-body"><span class="recipe-category-line">${escapeHtml(label)}</span><h3 class="recipe-title">${escapeHtml(r.name)}</h3><div class="recipe-meta recipe-meta-simple"><span>⏱ ${times||'—'} min</span><span>${escapeHtml(supportBadge(r))}</span></div></div></article>`;
}
function bindRecipeCards(root=document){
  root.querySelectorAll('[data-open-card]').forEach(card=>{card.onclick=e=>{if(e.target.closest('[data-fav],[data-missing-shop]'))return;openRecipe(card.dataset.openCard);};card.onkeydown=e=>{if((e.key==='Enter'||e.key===' ')&&!e.target.closest('button')){e.preventDefault();openRecipe(card.dataset.openCard);}};});
...
function openRecipe(id,open=true){
  ...
  $('#drawerContent').innerHTML=`<div class="drawer-hero cooking-first-hero"><div class="drawer-cover ${image?'has-image':'no-image'}">${image?`<img class="${r.imageFit==='contain'?'contain':''}" src="${escapeHtml(image)}" alt="${escapeHtml(r.name)}">`:`<div class="drawer-placeholder"><span>${categoryIcon[r.category]||'🍽️'}</span><strong>${escapeHtml(r.category)}</strong></div>`}</div><div class="drawer-head"><span class="kicker">${escapeHtml(r.category)}</span><h2 id="drawerTitle">${escapeHtml(r.name)}</h2><div class="drawer-meta primary-meta"><span>⏱ ${totalTime(r)||'—'} min</span><span>👥 ${state.currentServings} pers.</span><span>${escapeHtml(supportBadge(r))}</span>${r.cuisine?`<span>🌍 ${escapeHtml(r.cuisine)}</span>`:''}${r.originType?`<span>${escapeHtml(r.originType)}</span>`:''}</div><p>${escapeHtml(r.description||'')}</p><div class="drawer-actions main-cook-actions"><button class="primary big-cook-btn" id="startCookBtn">▶ Commencer à cuisiner</button></div><div class="recipe-secondary-actions"><button id="drawerFavBtn">${fav?'♥ Favori':'♡ Favori'}</button><button id="drawerShopBtn">+ Courses</button><button id="drawerPlanBtn">▤ Planifier</button><button id="drawerExportBtn">Créer le visuel</button></div></div></div>
  ...
  <section class="drawer-section full-width-section steps-main"><span class="kicker">PRÉPARATION</span><h3>Préparation pas à pas</h3><div class="steps-list compact-steps">${steps.map((st,i)=>`<div class="step-box"><strong>${i+1}. ${escapeHtml(st.title||'Étape')}</strong><p>${escapeHtml(st.text||'')}</p>${st.durationSec?`<small>⏱ ${Math.max(1,Math.round(st.durationSec/60))} min</small>`:''}</div>`).join('')}</div></section>
