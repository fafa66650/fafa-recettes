const STORAGE_KEY='fafatraining-recettes-v10';
...
const state={recipes:[],menus:[],prefs:{...DEFAULT_PREFS},filters:{query:'',category:'',time:'',equipment:'',style:'',favoritesOnly:false},visible:48,currentRecipe:null,currentServings:4,currentDrawerMode:'tm6',cook:{recipe:null,steps:[],index:0,timerLeft:0,timerInitial:0,timerId:null}};
...
const modeLabel=m=>m==='tm6'?'Thermomix TM6':m==='classic'?'Cuisine classique':'Autre appareil';
...
const homeStyles=['Cuisine express','Cuisine facile','Cuisine du quotidien','Cuisine familiale','Cuisine maison','Cuisine légère','Cuisine protéinée','Cuisine plaisir','Cuisine végétale','Cuisine économique','Batch cooking'];
const categoryIcon={
 'Boissons & smoothies':'🥤','Petit-déjeuner':'🥣','Entrées & salades':'🥗','Poulet & volaille':'🍗','Viandes':'🥩','Poissons & fruits de mer':'🐟','Végétarien':'🌿','Pâtes, riz & céréales':'🍝','Soupes & veloutés':'🍲','Four & gratins':'🔥','Desserts':'🍰','Collations':'🍎','Sauces & bases':'🥣','Repas protéinés':'💪','Recettes légères':'🥗',
 'Poulet':'🍗','Viande':'🥩','Poisson':'🐟','Pâtes':'🍝','Riz':'🍚','Soupe':'🍲','Légumes':'🥦','Dessert':'🍰','Protéiné':'💪','Perte de poids':'🥗','Snack sain':'🍎','Batch cooking':'📦','Fruits & smoothies':'🥤'
};

function loadPrefs(){
  try{const x=JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}');state.prefs={...DEFAULT_PREFS,...x};}
  catch{state.prefs={...DEFAULT_PREFS};}
  state.prefs.shopping=(state.prefs.shopping||[]).map(x=>typeof x==='string'?{id:x,servings:null}:x).filter(x=>x&&x.id);
}
...
async function init(){
  ...
  try{
    ...
    state.recipes=await rr.json(); state.menus=await mr.json();
  }catch(e){
    document.body.innerHTML='<div style="padding:30px;color:white;font-family:sans-serif">Impossible de charger les données. Ouvre l’application via un petit serveur web ou depuis son hébergement PWA.</div>'; return;
  }
  buildStaticUI(); bindEvents(); applyModeButtons(); renderHome(); renderRecipes(); renderMenus(); renderPantry(); renderShopping();
  navigate(state.prefs.lastPage && ['home','recipes','menus','fridge','shopping'].includes(state.prefs.lastPage)?state.prefs.lastPage:'home',false);
}

function buildStaticUI(){
  $('#homeRecipeCount').textContent=state.recipes.length;
  const styles=homeStyles.filter(s=>state.recipes.some(r=>(r.styles||[]).includes(s)));
  $('#homeStyleChips').innerHTML=styles.map(s=>`<button class="style-chip" data-home-style="${escapeHtml(s)}">${escapeHtml(s)}</button>`).join('');
  $('#recipeStyleChips').innerHTML=['Toutes',...styles].map((s,i)=>`<button class="style-chip ${i===0?'active':''}" data-recipe-style="${i===0?'':escapeHtml(s)}">${escapeHtml(s)}</button>`).join('');
  const cats=uniq(state.recipes.map(r=>r.category)).sort((a,b)=>a.localeCompare(b,'fr'));
  $('#categoryFilter').innerHTML='<option value="">Toutes les catégories</option>'+cats.map(c=>`<option value="${escapeHtml(c)}">${escapeHtml(c)}</option>`).join('');
...
function bindEvents(){
  ...
  $('#settingsBtn').onclick=()=>openSettings(true); $$('[data-close-settings]').forEach(x=>x.onclick=()=>openSettings(false));
  ...
  $('#resetPrefsBtn').onclick=()=>{state.prefs={...DEFAULT_PREFS,favorites:state.prefs.favorites,shopping:state.prefs.shopping,shoppingDone:state.prefs.shoppingDone,pantry:state.prefs.pantry};savePrefs();buildStaticUI();bindDynamicSettings();applyModeButtons();renderAll();toast('Préférences réinitialisées')};
...
function bindDynamicSettings(){
  ...
  $$('[data-home-style]').forEach(b=>b.onclick=()=>{state.filters.style=b.dataset.homeStyle;$('#recipeStyleChips').querySelectorAll('.style-chip').forEach(x=>x.classList.toggle('active',x.dataset.recipeStyle===state.filters.style));navigate('recipes');renderRecipes(true)});
...
function navigate(page,scroll=true){
  $$('.page').forEach(p=>p.classList.toggle('active',p.dataset.page===page)); $$('.nav-item').forEach(b=>b.classList.toggle('active',b.dataset.nav===page));
  state.prefs.lastPage=page;savePrefs(); if(scroll)window.scrollTo({top:0,behavior:'smooth'});
  if(page==='fridge')renderFridgeResults(); if(page==='shopping')renderShopping();
}
function setPreferredMode(mode){state.prefs.mode=mode;state.currentDrawerMode=mode;savePrefs();applyModeButtons();if(state.currentRecipe)openRecipe(state.currentRecipe.id,false);toast(`Mode : ${modeLabel(mode)}`)}
...
function recipeHay(r){return norm([r.name,r.category,r.description,(r.tags||[]).join(' '),(r.styles||[]).join(' '),r.ingredients.map(i=>i.name).join(' ')].join(' '));}
...
function resetFilters(){state.filters={query:'',category:'',time:'',equipment:'',style:'',favoritesOnly:false};$('#recipeSearch').value='';$('#categoryFilter').value='';$('#timeFilter').value='';$('#equipmentFilter').value='';$('#favoritesOnly').checked=false;$$('[data-recipe-style]').forEach(b=>b.classList.toggle('active',b.dataset.recipeStyle===''));renderRecipes(true)}

function renderHome(){
  const featured=state.recipes.filter(r=>r.featured&&passesRestrictions(r)).slice(0,8);$('#featuredGrid').innerHTML=featured.map(recipeCard).join('');bindRecipeCards($('#featuredGrid'));
  const counts={};state.recipes.filter(passesRestrictions).forEach(r=>counts[r.category]=(counts[r.category]||0)+1);
  const cats=Object.entries(counts).sort((a,b)=>b[1]-a[1]).slice(0,12);
  $('#homeCategories').innerHTML=cats.map(([c,n])=>`<button class="category-card" data-home-cat="${escapeHtml(c)}"><span>${categoryIcon[c]||'🍽️'}</span><strong>${escapeHtml(c)}</strong><small>${n} recettes</small></button>`).join('');
  ...
  $('#homeMenus').innerHTML=state.menus.slice(0,4).map(menuCard).join('');bindMenuCards($('#homeMenus'));
  const fav=state.recipes.filter(r=>state.prefs.favorites.includes(r.id)).slice(0,4);$('#homeFavoritesSection').classList.toggle('hidden',!fav.length);$('#homeFavorites').innerHTML=fav.map(recipeCard).join('');bindRecipeCards($('#homeFavorites'));
}

function recipeCard(r,extra=''){
  const fav=state.prefs.favorites.includes(r.id);const image=imgSrc(r);const times=totalTime(r);const style=(r.styles||[])[0]||'Cuisine maison';
  return `<article class="recipe-card" ${extra}><div class="recipe-image ${image?'':'no-image'}">${image?`<img src="${escapeHtml(image)}" alt="${escapeHtml(r.name)}" loading="lazy">`:`<span>${r.emoji||categoryIcon[r.category]||'🍽️'}</span>`}<button class="fav-btn ${fav?'active':''}" data-fav="${r.id}" aria-label="Favori">${fav?'♥':'♡'}</button></div><div class="recipe-body"><div class="recipe-topline"><span class="tiny-badge ${r.featured?'featured':''}">${r.featured?'FAFATRAINING':escapeHtml(r.category)}</span><span class="tiny-badge">${escapeHtml(style)}</span></div><h3 class="recipe-title">${escapeHtml(r.name)}</h3><div class="recipe-desc">${escapeHtml(r.description||'Recette détaillée avec plusieurs modes de préparation.')}</div><div class="recipe-meta"><span>⏱ ${times||'—'} min</span><span>👥 ${r.servings}</span><span>${(r.equipment||[]).includes('TM6')?'TM6 ✓':'Classique'}</span></div><div class="card-actions"><button class="open-btn" data-open="${r.id}">Ouvrir</button><button class="shop-btn" data-shop="${r.id}">+ Courses</button></div></div></article>`;
}
...
function renderRecipes(resetVisible=false){
  if(resetVisible)state.visible=48;const arr=filteredRecipes();$('#recipesCountLabel').textContent=`${arr.length} recette${arr.length>1?'s':''} trouvée${arr.length>1?'s':''}`;$('#recipesGrid').innerHTML=arr.slice(0,state.visible).map(recipeCard).join('');bindRecipeCards($('#recipesGrid'));$('#loadMoreBtn').classList.toggle('hidden',state.visible>=arr.length);
}
function toggleFavorite(id){const a=state.prefs.favorites;state.prefs.favorites=a.includes(id)?a.filter(x=>x!==id):[...a,id];savePrefs();renderHome();renderRecipes();if(state.currentRecipe&&state.currentRecipe.id===id)openRecipe(id,false);toast(state.prefs.favorites.includes(id)?'Ajouté aux favoris':'Retiré des favoris')}

function menuCard(m){const rs=m.recipes.map(id=>state.recipes.find(r=>r.id===id)).filter(Boolean);return `<article class="menu-card"><span class="kicker">MENU COMPLET</span><h3>${escapeHtml(m.title)}</h3><p>${escapeHtml(m.subtitle||'')}</p><div class="menu-list">${rs.map((r,i)=>`<div><span>${i+1}</span><small>${escapeHtml(r.name)}</small></div>`).join('')}</div><div class="menu-actions"><button data-open-menu-first="${m.id}">Voir</button><button class="primary" data-shop-menu="${m.id}">+ Courses</button></div></article>`}
function renderMenus(){$('#menusGrid').innerHTML=state.menus.map(m=>`<article class="menu-card-full"><span class="kicker">${escapeHtml((m.tags||[])[0]||'MENU')}</span><h3>${escapeHtml(m.title)}</h3><p>${escapeHtml(m.subtitle||'')}</p><div class="menu-list">${m.recipes.map((id,i)=>{const r=state.recipes.find(x=>x.id===id);return r?`<div><span>${i+1}</span><small><strong>${i===0?'Entrée':i===1?'Plat':'Dessert'} :</strong> ${escapeHtml(r.name)}</small></div>`:''}).join('')}</div><div class="menu-actions"><button data-open-menu-first="${m.id}">Ouvrir une recette</button><button class="primary" data-shop-menu="${m.id}">Ajouter le menu aux courses</button></div></article>`).join('');bindMenuCards($('#menusGrid'))}
function bindMenuCards(root){root.querySelectorAll('[data-shop-menu]').forEach(b=>b.onclick=()=>addMenuToShopping(b.dataset.shopMenu));root.querySelectorAll('[data-open-menu-first]').forEach(b=>b.onclick=()=>{const m=state.menus.find(x=>x.id===b.dataset.openMenuFirst);if(m?.recipes[0])openRecipe(m.recipes[0])})}
function addMenuToShopping(id){const m=state.menus.find(x=>x.id===id);if(!m)return;m.recipes.forEach(rid=>{const r=state.recipes.find(x=>x.id===rid);if(r&&!state.prefs.shopping.some(x=>x.id===rid))state.prefs.shopping.push({id:rid,servings:r.servings||4})});savePrefs();renderShopping();toast('Menu ajouté aux courses')}
...
function renderAll(){renderHome();renderRecipes();renderMenus();renderPantry();renderFridgeResults();renderShopping();}
