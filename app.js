...
function renderRecipeLibrary(){
  ...
  box.querySelectorAll('[data-library-category]').forEach(b=>b.onclick=()=>{state.filters.category=b.dataset.libraryCategory;state.filters.subtype='';state._showAllRecipes=false;$('#categoryFilter').value=state.filters.category;renderRecipes(true);window.scrollTo({top:0,behavior:'smooth'});});
...
function personalizedRecipes(){
  ...
  const buckets=[
    r=>isMainCourseCategory(r.category),
    r=>['Légumes & accompagnements','Entrées & salades','Soupes & veloutés','Végétarien'].includes(r.category),
    r=>['Petit-déjeuner','Desserts','Collations','Boissons & smoothies'].includes(r.category)
  ];
  const picked=[];
  for(const test of buckets){const found=scored.find(x=>test(x.r)&&!picked.some(p=>p.id===x.r.id)&&!picked.some(p=>p.category===x.r.category));if(found)picked.push(found.r);}
  for(const x of scored){if(picked.length>=3)break;if(!picked.some(p=>p.id===x.r.id)&&!picked.some(p=>p.category===x.r.category))picked.push(x.r);}
...
function recipeCard(r,extra=''){
  ...
  return `<article class="recipe-card recipe-card-clickable" data-open-card="${r.id}" tabindex="0" role="button" aria-label="Ouvrir ${escapeHtml(r.name)}" ${extra}><div class="recipe-image ${image?'':'no-image'}">${image?`<img src="${escapeHtml(image)}" alt="${escapeHtml(r.name)}" loading="lazy">`:`<div class="recipe-placeholder"><span>${r.emoji||categoryIcon[r.category]||'🍽️'}</span><strong>${escapeHtml(label)}</strong></div>`}<button class="fav-btn ${fav?'active':''}" data-fav="${r.id}" aria-label="${fav?'Retirer des':'Ajouter aux'} favoris">${fav?'♥':'♡'}</button></div><div class="recipe-body"><span class="recipe-category-line">${escapeHtml(label)}</span><h3 class="recipe-title">${escapeHtml(r.name)}</h3><div class="recipe-meta recipe-meta-simple"><span>⏱ ${times||'—'} min</span><span>${escapeHtml(supportBadge(r))}</span></div></div></article>`;
}
