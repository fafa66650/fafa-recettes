...
function bindEvents(){
  ...
  $('#addCustomPantryBtn').onclick=addCustomPantryIngredient;$('#customPantryInput').addEventListener('keydown',e=>{if(e.key==='Enter')addCustomPantryIngredient();});$('#closePantryGroupBtn').onclick=()=>{state.pantryGroup='';renderPantry();};
...
function exportUserData(){
  const payload={app:'FAFATRAINING Recettes',version:'18',date:new Date().toISOString(),prefs:state.prefs};const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='fafatraining-recettes-sauvegarde.json';a.click();URL.revokeObjectURL(a.href);toast('Sauvegarde créée');
}
...
function renderPantry(){
  const all=pantryIngredients(),q=norm($('#fridgeSearch')?.value||'');const selected=state.prefs.pantry||[];
  ...
  let list=[];let title='';if(q){list=all.filter(x=>norm(x).includes(q)).slice(0,80);title=`Résultats pour « ${$('#fridgeSearch').value.trim()} »`;state.pantryGroup='__search__';}else if(state.pantryGroup&&state.pantryGroup!=='__search__'){list=groups[state.pantryGroup]||[];title=state.pantryGroup;}
...
function pantryGroupFor(name){const h=norm(name);if(/poulet|dinde|boeuf|veau|porc|agneau|jambon|steak|viande/.test(h))return'Viandes & volaille';if(/saumon|cabillaud|colin|merlu|dorade|bar$|thon|sardine|maquereau|poisson|crevette|moule|calamar|poulpe|crabe/.test(h))return'Poissons & fruits de mer';if(/pomme|poire|banane|citron|orange|mangue|kiwi|fraise|myrtille|framboise|ananas|peche|abricot|raisin|melon|pasteque|fruit/.test(h))return'Fruits';if(/courgette|aubergine|brocoli|chou|carotte|poireau|epinard|haricot vert|poivron|tomate|concombre|champignon|fenouil|celeri|asperge|artichaut|betterave|panais|navet|courge|butternut|patate douce|gombo|blette|petit pois|radis|endive|salade|roquette|mache|avocat/.test(h))return'Légumes';if(/oeuf|lait|yaourt|fromage|creme|beurre|ricotta|feta|mozzarella|parmesan|skyr|tofu|tempeh/.test(h))return'Œufs, laitages & alternatives';if(/pate|riz|quinoa|boulgour|semoule|couscous|millet|sarrasin|avoine|farine|pain|polenta|orge|gnocchi/.test(h))return'Pâtes, riz & céréales';if(/lentille|pois chiche|haricot rouge|haricot blanc|haricot noir|pois casse|feve/.test(h))return'Légumineuses';if(/persil|menthe|basilic|coriandre|thym|origan|romarin|cumin|paprika|curry|cannelle|gingembre|curcuma|muscade|piment|herbe|epice/.test(h))return'Herbes & épices';if(/huile|vinaigre|moutarde|miel|sirop|sauce|tomate concassee|coulis|bouillon|cacao|chocolat|sucre|sel|poivre|olive|capre|tahini|puree de cacahuete/.test(h))return'Épicerie, sauces & condiments';return'Autres & placard';}
function pantryGroups(all){const groups={};all.forEach(x=>(groups[pantryGroupFor(x)]??=[]).push(x));const order=['Légumes','Fruits','Viandes & volaille','Poissons & fruits de mer','Œufs, laitages & alternatives','Pâtes, riz & céréales','Légumineuses','Épicerie, sauces & condiments','Herbes & épices','Autres & placard'];const out={};order.forEach(g=>{if(groups[g])out[g]=groups[g].sort((a,b)=>a.localeCompare(b,'fr'));});return out;}
function pantryGroupIcon(g){return {'Légumes':'🥕','Fruits':'🍎','Viandes & volaille':'🥩','Poissons & fruits de mer':'🐟','Œufs, laitages & alternatives':'🥚','Pâtes, riz & céréales':'🌾','Légumineuses':'🫘','Épicerie, sauces & condiments':'🧂','Herbes & épices':'🌿','Autres & placard':'🫙'}[g]||'🍽️';}
