...
function renderRecipeLibrary(){
  ...
  const cbox=$('#recipeCuisineHighlights');if(cbox){
    ...
    cbox.innerHTML=groups.map(([g,n,detail])=>`<button class="cuisine-family-card ${g==='Créations maison'?'home-cuisine-card':''}" data-library-cuisine-group="${escapeHtml(g)}"><span><strong>${escapeHtml(g)}</strong><small>${escapeHtml(detail)}</small></span><span class="cuisine-count">${n} recettes</span><b>→</b></button>`).join('');
...
function openRecipe(id,open=true){
  ...
  const methodNote=state.currentDrawerMode==='tm6'?(r.tm6Status==='full'?'Toutes les étapes de cette méthode sont prévues pour le Thermomix TM6.':'Le TM6 intervient uniquement sur les étapes où il apporte une aide réelle ; les autres gestes restent classiques.'):`Matériel principal : ${supportBadge(r)}. Les étapes ci-dessous correspondent à la méthode classique.`;
  $('#drawerContent').innerHTML=`<div class="drawer-hero cooking-first-hero ${image?'':'no-cover'}">${cover}<div class="drawer-head"><span class="kicker">${escapeHtml(r.category)} · ${escapeHtml(r.subtype||'Recette')}</span><h2 id="drawerTitle">${escapeHtml(r.name)}</h2><div class="drawer-meta primary-meta"><span>⏱ ${totalTime(r)||'—'} min</span><span>👥 ${state.currentServings} pers.</span><span>◉ ${escapeHtml(r.difficulty||'Facile')}</span><span>🍳 ${escapeHtml(supportBadge(r))}</span>${origin?`<span>🌍 ${escapeHtml(origin)}</span>`:''}</div><p class="recipe-intro">${escapeHtml(r.description||'')}</p><div class="drawer-actions main-cook-actions"><button class="primary big-cook-btn" id="startCookBtn">▶ Commencer à cuisiner</button></div><div class="recipe-secondary-actions"><button id="drawerFavBtn">${fav?'♥ Favori':'♡ Favori'}</button><button id="drawerShopBtn">+ Courses</button><button id="drawerPlanBtn">▤ Planifier</button><button id="drawerExportBtn">Créer le visuel</button></div></div></div>
